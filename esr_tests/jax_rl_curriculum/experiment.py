...
def experiment(config: DictConfig):
    try:
        ...
        domain_randomization_params =  OmegaConf.to_container(config.randomization_config["randomization_params"], resolve=True)
        ...
        if config.checkpoint_path is not None: 
            old_agent_conf, agent_state = PPOJax.load_agent(config.checkpoint_path)
            train_fn = PPOJax.build_resume_train_fn(env, agent_conf, mh=mh, agent_state=agent_state)
        else:
        ...
        print(f"CHECKPOINT_PATH:{save_path}") # Needs to be set to continue training
